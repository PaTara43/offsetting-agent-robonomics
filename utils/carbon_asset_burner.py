...
from .constants import CARBON_ASSET_ID, STATEMINE_REMOTE_WS
...
def burn_carbon_asset(seed: str, tokens_to_burn: float) -> tuple:
    ...
    return (
        ...
        f"https://polkadot.js.org/apps/?rpc={STATEMINE_REMOTE_WS[: STATEMINE_REMOTE_WS.find('://')]}%3A%2F%2F"
        f"{STATEMINE_REMOTE_WS[STATEMINE_REMOTE_WS.find('://')+3:]}#/explorer/query/{receipt.block_hash}",
    )
